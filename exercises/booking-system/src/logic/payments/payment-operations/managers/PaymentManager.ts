...
export default class PaymentManager implements PaymentOperationsProvider {
  ...
  public cancelPayment(paymentId: string): void {
    ...
    // send refund command
    const externalPaymentProcessingAdapter: ExternalPaymentProcessingAdapter | null =
      this.findExternalPaymentProcessingAdapter(paymentDetails.paymentProcessingServiceName);
    if (! externalPaymentProcessingAdapter) {
